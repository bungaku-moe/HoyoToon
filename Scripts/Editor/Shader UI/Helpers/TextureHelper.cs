    ...
    public class TextureHelper : Editor
    {
        public static Gradient GetGradient(Texture texture)
        {
            if (texture != null)
            {
                ...
                {
                    HoyoToonLogs.LogDebug(texture.name + " Gradient loaded from file.");
                    Gradient g = Parser.Deserialize<Gradient>(gradient_data_string);
                    return g;
                }
                HoyoToonLogs.LogDebug(texture.name + " Converted into Gradient.");
        ...
        public static Texture SaveTextureAsPNG(Texture2D texture, string path, TextureData settings = null)
        {
            ...
            HoyoToonLogs.LogDebug("Texture saved at \"" + path + "\".");
        ...
        public static Texture2D LoadTGA(string TGAFile, bool displayProgressbar = false)
        {
            using (BinaryReader r = new BinaryReader(File.Open(TGAFile, FileMode.Open)))
            {
                ...
                if (ImageType == 0)
                {
                    ...
                    HoyoToonLogs.ErrorDebug("Unsupported TGA file! No image data");
                }
                else if (ImageType == 3 | ImageType == 11)
                {
                    ...
                    HoyoToonLogs.ErrorDebug("Unsupported TGA file! Not truecolor");
                }
                else if (ImageType == 9 | ImageType == 10)
                {
                    ...
                    HoyoToonLogs.ErrorDebug("Unsupported TGA file! Colormapped");
...
#pragma warning restore CS0618
                        ...
                        if (BPP.ContainsKey(textureFormat))
                        {
                            ...
                            //HoyoToonLogs.LogDebug(bytesCount);
                        }
                        else
                        {
                            HoyoToonLogs.WarningDebug("[VRAM] Does not have BPP for " + textureFormat);
                        }
                    }
                    else
                    {
